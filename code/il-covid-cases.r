...
il.cases <- ggplot(stateDF) +
  geom_bar(aes(y = confirmed_cases, x = ymd(testDate)), stat="identity", color=NA, fill="#617A89", alpha = 0.35) +
  ...
  geom_line(aes(y=confirmed_cases_7day_ma, x = ymd(testDate)), size = 1, na.rm = TRUE, color = "#617A89") +
  scale_x_date(
    date_labels = "%Y-%m-%d",
    date_breaks = "1 month",
    expand = c(0,0.5)
...
labs(
  ...
  subtitle=str_glue("Number of confirmed cases and 7-day moving average, as of {today.state}"),
...
il.chg.cases <- ggplot(stateDF) +
  geom_bar(aes(y = daily_confirmed_cases, x = ymd(testDate)), stat="identity", color=NA, fill="#617A89", alpha = 0.35) +
  ...
  geom_line(aes(y=daily_confirmed_cases_7day_ma, x = ymd(testDate)), size = 1, na.rm = TRUE, color = "#617A89") +
  scale_x_date(
    date_labels = "%Y-%m-%d",
    date_breaks = "1 month",
    expand = c(0,0.25)
...
labs(
  title="Illinois COVID-19 Case Count",
  subtitle=str_glue("Change in the number of confirmed cases and 7-day moving average, as of {today.state}"),
  caption="Author: Chris Goodman (@cbgoodman), Data: IL Department of Public Health (https://dph.illinois.gov/).",
  y=NULL,
  x=NULL) +
theme_minimal(base_family="Public Sans Thin") +
