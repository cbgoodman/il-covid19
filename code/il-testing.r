...
il.tested <- ggplot(stateDF) +
  geom_bar(aes(y = total_tested, x = ymd(testDate)), stat="identity", color=NA, fill="#617A89", alpha = 0.35) +
...
#  family="Public Sans Light", lineheight=0.95, size=2, color="#2b2b2b") +
  geom_line(aes(y=total_tested_7day_ma, x = ymd(testDate)), size = 1, na.rm = TRUE, color = "#617A89") +
  scale_x_date(
    date_labels = "%Y-%m-%d",
    date_breaks = "1 month",
    expand = c(0,0)
...
ggsave(plot=il.tested, "il-covid-tests.png", path="./images", width=10, height=6, units="in", dpi="retina", bg = "#ffffff")


# Change in Cases plot
il.chg.tested <- ggplot(stateDF) +
  geom_bar(aes(y = daily_total_tested, x = ymd(testDate)), stat="identity", color=NA, fill="#617A89", alpha = 0.35) +
  ...
  geom_line(aes(y=daily_total_tested_7day_ma, x = ymd(testDate)), size = 1, na.rm = TRUE, color = "#617A89") +
  scale_x_date(
    date_labels = "%Y-%m-%d",
    date_breaks = "1 month",
    expand = c(0,0)
...
theme(plot.subtitle=element_text(family="Public Sans Italic", size=9))+
theme(plot.caption=element_text(size=8, hjust=0, margin=margin(t=15)))
ggsave(plot=il.chg.tested, "il-covid-chg-test.png", path="./images", width=10, height=6, units="in", dpi="retina", bg = "#ffffff")

# Positive rate
il.positive <- ggplot(stateDF) +
  geom_line(aes(y = daily_positive_rate, x = ymd(testDate)), color="#E69F00") +
...
theme(plot.subtitle=element_text(family="Public Sans Italic", size=9))+
theme(plot.caption=element_text(size=8, hjust=0, margin=margin(t=15)))
ggsave(plot=il.positive, "il-covid-positive-rate.png", path="./images", width=10, height=6, units="in", dpi="retina", bg = "#ffffff")
