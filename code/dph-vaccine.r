...
statewide_vaccine %>%
ggplot() +
  geom_bar(aes(y = AdministeredCountChange, x = ymd(Report_Date)), stat="identity", color = NA, fill="#617A89", alpha=0.25) +
  ...
  geom_line(aes(y=AdministeredCountChange_7day_ma, x = ymd(Report_Date)), size = 1, na.rm = TRUE, color = "#617A89") +
  scale_x_date(
    date_labels = "%Y-%m-%d",
    date_breaks = "1 month",
    expand = c(0,0.5)
