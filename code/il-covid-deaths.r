...
il.deaths <- ggplot(stateDF) +
  ...
  geom_line(aes(deaths.ma.7day, x = ymd(date)), size = 1, na.rm = TRUE, color = "#7F7F7F") +
  scale_x_date(
    date_labels = "%Y-%m-%d",
    date_breaks = "3 days",
    expand = c(0,0)
...
il.chg.deaths <- ggplot(stateDF) +
  geom_bar(aes(y = chg.deaths, x = ymd(date)), stat="identity", fill="#E69F00") +
  #geom_text(aes(y = chg.deaths, x = ymd(date), label = comma(chg.deaths, accuracy=1)), position=position_dodge(width=0.9), vjust=-0.25,
  #family="Public Sans Thin", lineheight=0.95, size=2, color="#2b2b2b") +
  geom_line(aes(y=deaths.ma.7day.chg, x = ymd(date)), size = 1, na.rm = TRUE, color = "#7F7F7F") +
  scale_x_date(
    date_labels = "%Y-%m-%d",
    date_breaks = "3 days",
    expand = c(0,0)
