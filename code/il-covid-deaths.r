# Create two figures measuring total deaths and daily change in
# deathes from COVID-19 in Illinois

# Cases plot
il.deaths <- ggplot(stateDF) +
  geom_bar(aes(y = deaths, x = ymd(date)), stat="identity", fill="#E69F00") +
  ...
  geom_line(aes(deaths_7day_ma, x = ymd(date)), size = 1, na.rm = TRUE, color = "#7F7F7F") +
  scale_x_date(
    date_labels = "%Y-%m-%d",
    date_breaks = "3 days",
    expand = c(0,0)
...
ggsave(plot=il.deaths, "il-covid-deaths.png", path="./images", width=10, height=6, units="in", dpi="retina")


# Change in Cases plot
il.chg.deaths <- ggplot(stateDF) +
  geom_bar(aes(y = daily_deaths, x = ymd(date)), stat="identity", fill="#E69F00") +
  #geom_text(aes(y = chg.deaths, x = ymd(date), label = comma(chg.deaths, accuracy=1)), position=position_dodge(width=0.9), vjust=-0.25,
  #family="Public Sans Thin", lineheight=0.95, size=2, color="#2b2b2b") +
  geom_line(aes(y=daily_deaths_7day_ma, x = ymd(date)), size = 1, na.rm = TRUE, color = "#7F7F7F") +
  scale_x_date(
    date_labels = "%Y-%m-%d",
    date_breaks = "3 days",
    expand = c(0,0)
...
theme(plot.subtitle=element_text(family="Public Sans Italic", size=9))+
theme(plot.caption=element_text(size=8, hjust=0, margin=margin(t=15)))
ggsave(plot=il.chg.deaths, "il-covid-chg-deaths.png", path="./images", width=10, height=6, units="in", dpi="retina")
